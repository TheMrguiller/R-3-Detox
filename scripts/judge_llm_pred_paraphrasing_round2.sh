...
data_path="data/interim/human_eval_paraphrasing_judgellm_round2/"
save_path="data/processed/human_eval_paraphrasing_judgellm_round2/"
...
for param in "${params[@]}"; do
    ...
    for file in "${file_list[@]}"; do
        ...
        if ! [[ "$file_name" == "${base_name}" ]]; then
            echo "Skipping file ${base_name} for parameter ${param}"
            continue
        fi
        srun --ntasks=1 --cpus-per-task=32 python src/evaluation/JudgeLM/JudgeLM/judgelm/llm_judge/gen_model_judgement_multi.py \
            ...
            --temperature 0 \
            --answer-num 3 \
            --if-fast-eval 1 > logs/system/generating_judge_pred_${SLURM_JOB_ID}_${param}.txt
    done
done
