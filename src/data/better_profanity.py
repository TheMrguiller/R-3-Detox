...
class ProfanityExtractor:
    ...
    def detect_censor_words(self, text):
        if not isinstance(text, str):
            text = str(text)
        if not self.CENSOR_WORDSET:
            self.load_censor_words()
        return self._detect_swear_words(text)
    
    def _detect_swear_words(self, text):
...
if __name__ == '__main__':
    pe = ProfanityExtractor()
    profane_text = "You are a fucking idiot, you moron motherfucker"
    print(pe.detect_censor_words(profane_text))