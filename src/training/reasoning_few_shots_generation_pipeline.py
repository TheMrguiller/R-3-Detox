...
def extract_vllm_reasoning(config_path:str,df:pd.DataFrame,toxic_prompt_template:str,non_toxic_prompt_template:str,batch_size:int=8,num_return_sequences:int=4):
    ...
    for idx in tqdm(range(0, len(df), batch_size),desc="Generating reasoning"):
        ...
        toxic_words = batch['shap_values'].tolist()
        user_messages=generate_messages(toxic_prompt_template,non_toxic_prompt_template,original_messages,paraphrases,toxic_words,labels)
        llm.sampling_params.temperature = 0.0
        llm.sampling_params.top_p=1.0
        responses_greedy = llm.query(user_messages=user_messages)
        llm.sampling_params.temperature = 0.7
        llm.sampling_params.top_p=0.9
        user_messages =[[user_message]*num_return_sequences for user_message in user_messages]
        user_messages = [item for sublist in user_messages for item in sublist]
        responses_sample = llm.query(user_messages=user_messages)
        responses_sample = [responses_sample[i:i+4] for i in range(0, len(responses_sample), num_return_sequences)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_name",default="qwq_preview",choices=["marco-o1","openO1","qwq_preview","skywork-o1"],help="Name of the c to use")
    ...
    # print(f"Time elapsed: {time_elapsed_minutes} minutes")
    ...
    df.to_csv(proyect_path + f'data/interim/few_shot_reasoning/few_shot_reasoning_{config_name}.csv',index=False)
