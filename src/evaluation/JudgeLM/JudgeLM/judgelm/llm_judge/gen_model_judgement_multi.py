...
def run_eval(
    ...
    for i in range(0, len(questions), chunk_size):
        ...
        print(f"ans_handles append {i}")

    if use_ray:
        ray.get(ans_handles)

def truncate_text_to_tokens(text:str, tokenizer, max_tokens=900):
    """
    Truncate text to a maximum number of tokens without requiring decoding.

    Parameters:
        text (str): The input text to process.
        tokenizer: The tokenizer object to use for tokenizing the text.
        max_tokens (int): The maximum allowed tokens (default: 2500).

    Returns:
        str: The truncated text that fits within the token limit.
    """
    logger.info(f"Calculating token count for text.")
    # Tokenize the text
    tokens = tokenizer.tokenize(text)

    # Check if token count exceeds the limit
    if len(tokens) > max_tokens:
        logger.info(f"Truncating text to {max_tokens} tokens.")
        # print(f"Tokens:{tokens}")
        # Truncate tokens to the specified limit
        text_split=text.split()
        logger.info(f"length of text_split: {len(text_split)}")
        text_split=text_split[:256]
        truncated_text = " ".join(text_split)
        tokens = tokenizer.tokenize(truncated_text)
        logger.info(f"length of tokens: {len(tokens)}")
        # print(f"Tokens:{tokens}")
        return truncated_text
    else:
        logger.info(f"Text fits within the token limit.{len(tokens)}")
        return text
...
def get_model_answers(
    ...
    print(f"Max memory: {max_gpu_memory}")
    if model_path!="BAAI/JudgeLM-33B-v1.0":
    ...
    logger.info(f"Available total memory: {total / 1024 ** 2}")
    logger.info(f"Memory used by model:{mem_used_MB}")
    for q_i, question in tqdm(enumerate(questions)):
        ...
        logger.info(f"Parsing question {q_i}th.")
        question["answer1_body"] = truncate_text_to_tokens(question["answer1_body"], tokenizer)
        question["answer2_body"] = truncate_text_to_tokens(question["answer2_body"], tokenizer)
        # combine data_sample
        ...
        logger.info(f"Length of input_ids: {len(input_ids[0])}")
        if len(input_ids[0]) > 2048:
            logger.info(f"Length of input is greater than model's limit.")
        # print(data_sample)
        # input_ids = tokenizer([data_sample],max_length=4096,truncation=True).input_ids
        ...
        print(f"Preprocessing {q_i}th question.")
        # generate judgements
        ...
        logger.info(f"Preprocessing {q_i}th question done.")
        free, total = torch.cuda.mem_get_info(model.device)
        mem_used_MB = (total - free) / 1024 ** 2
        logger.info(f"Memory used by model in inference:{mem_used_MB}")
        ...
        del input_ids, output_ids, output
        gc.collect()
